...
class BeasiswaController extends Controller
{
    function index(Request $request) {
        $query = Submission::where('type', Submission::TYPES[9]);
        $submissionType = SubmissionOrderingService::getSubmissionTypeString(9);
        $submissions = SubmissionOrderingService::applyOrdering($query, $submissionType)->paginate(10);
        return view('admin.surat-rekomendasi.beasiswa.index', compact('submissions'));
    }

    function show(Request $request, Submission $submission) {
        return view('admin.surat-rekomendasi.beasiswa.show', compact('submission'));
    }
    ...
    function update(Request $request, Submission $submission) {
        ...
        return redirect()->route('admin.surat-rekomendasi.beasiswa.index')->with([
            'status' => 'success',
